...
<?php if($sortFields or $filterFields or $searchFields):?>
	  ...
	  <form id="navbarForm"
		action="<?php echo BASE_REQUEST . '/admin'; ?>_<?php echo isset($controller)?$controller->module: '';?>/searchFilter">
		<div class="row">
           <?php if ($searchFields) : ?>
              <div class="form-group col-xs-12">
				<label>Tìm theo </label><br /> <input type="text" name="keyword" class="form-control"
					placeholder="<?php if($Searchlabels){ echo $Searchlabels; } ?>" value="<?php echo isset($keyword)?$keyword: '';?>" onkeyup="pzk_list.search(this.value);" />
			</div>
            <?php endif; ?>
        <?php if ($filterFields) :?>
			<?php foreach ( $filterFields as $field ) :?>
                ...
                <div class="form-group col-xs-12">
					<label><?php echo isset($field['label'])?$field['label']: '';?></label><br /> 
					<select id="<?php echo isset($field['index'])?$field['index']: '';?>"
						name="<?php echo isset($field['index'])?$field['index']: '';?>" class="form-control"
					...
					<script type="text/javascript">
	                    ...
	                    $('#<?php echo isset($field['index'])?$field['index']: '';?>').val('<?php echo isset($status)?$status: '';?>');
	                </script>
				</div>
                <?php  elseif($field['type'] == 'select') : ?>
                    <div class="form-group col-xs-12">
						<label><?php echo isset($field['label'])?$field['label']: '';?></label><br /> 
						<select id="<?php echo isset($field['index'])?$field['index']: '';?>" name="<?php echo isset($field['index'])?$field['index']: '';?>" class="form-control" onchange="pzk_list.filter('<?php echo isset($field['type'])?$field['type']: '';?>', '<?php echo isset($field['index'])?$field['index']: '';?>', this.value);">
                            <?php
						...
						<script type="text/javascript">
                            ...
                            $('#<?php echo isset($field['index'])?$field['index']: '';?>').val('<?php echo isset($select)?$select: '';?>');
                        </script>
					</div>
               	<?php elseif($field['type'] == 'datetime'):?>
                	<div class="form-group col-xs-12">
                		...
                		<script type="text/javascript">
                            ...
                            $('#<?php echo isset($field['index'])?$field['index']: '';?>').val('<?php echo isset($datetime)?$datetime: '';?>');
                        </script>
                	</div>
                <?php else :?>
                	<?php 
	    ...
	    <?php if($sortFields) : ?>
		    	<div class="form-group col-xs-12">
					...
					<select id="orderBy" name="orderBy" class="form-control" onchange="pzk_list.changeOrderBy(this.value);">
	                    ...
	                    <option value="<?php echo isset($value)?$value: '';?>"><?php echo isset($label)?$label: '';?></option>
	                    <?php } ?>
	                </select>
					<script type="text/javascript">
	                	$('#orderBy').val('<?php echo isset($orderBy)?$orderBy: '';?>');
	                </script>
				</div>
	    <?php endif; ?>
       	<?php if($searchFields) :?>
		...
		<div class="row">
			<div class="form-group col-xs-12 hidden">
				...
				<script type="text/javascript">
                      $('#view').val('<?php echo isset($view)?$view: '';?>');
                  </script>
			</div>
			<div class="form-group col-xs-12 hidden">
				...
				<script type="text/javascript">
                      $('#columns').val('<?php echo isset($columns)?$columns: '';?>');
                  </script>
			</div>
			<div class="form-group col-xs-12 hidden">
				<label>&nbsp;</label><br />
			...
			<div style="margin-left: 15px;"
				...
				<button type="button" value="" name="submit_action"
					...
					onclick="window.location='/admin_<?php echo isset($data)?$data->getModule(): '';?>/add';">
